...
type SettingsTab = 'profile' | 'appearance' | 'server' | 'channels' | 'notifications' | 'permissions'

export function SettingsDialog({ channels, identity, onClose, onDeleteChannel, onSaveChannel, onSaveProfile, onSaveServer, onSetMuted, onUploadAvatar, channelPermissions, inviteLinks, members, onCreateInviteLink, onRevokeInviteLink, onSaveChannelPermissions, onSetMemberRole, categories, onCreateCategory, onLeaveServer, onMarkServerRead, onSaveServerNickname, server, serverMuted, userId }: SettingsDialogProps) {
  ...
  const [channelKind, setChannelKind] = useState<ChannelSummary['kind']>('text')
      ...
      <section className="server-dialog settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-dialog-title">
        ...
        <div className="settings-layout">
          <nav aria-label="Seções das configurações" className="settings-nav">
            {([['profile', 'Perfil'], ['appearance', 'Tema'], ['server', 'Servidor'], ['channels', 'Canais'], ['notifications', 'Notificações'], ['permissions', 'Permissões']] as const).map(([id, label]) => <button className={tab === id ? 'active' : ''} key={id} type="button" onClick={() => setTab(id)}>{label}</button>)}
          </nav>
