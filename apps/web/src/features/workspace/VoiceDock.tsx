  ...
  return (
    <aside className={floating ? 'voice-dock joined floating' : 'voice-dock joined'} aria-label="Conexao de voz">
      ...
      <footer className="voice-controls">
        <button aria-label={microphoneDisabled ? 'Microfone desativado pela moderação' : 'MIC'} className={microphoneOff ? 'disabled' : ''} disabled={microphoneDisabled} type="button" onClick={onToggleMicrophone}><span>{microphoneOff ? '×' : '⌁'}</span>MIC</button>
        <button aria-label={outputDisabled ? 'Áudio desativado pela moderação' : 'ÁUDIO'} className={outputOff ? 'disabled' : ''} disabled={outputDisabled} type="button" onClick={onToggleOutput}><span>{outputOff ? '×' : '◖'}</span>ÁUDIO</button>
