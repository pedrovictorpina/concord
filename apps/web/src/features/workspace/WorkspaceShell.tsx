  ...
  return (
    <main className={workspace.activeServer ? 'app-shell' : 'app-shell home-shell'}>
      ...
      {workspace.activeServer ? <><ChannelPanel activeChannelId={workspace.activeChannelId} activeVoiceChannelId={activeVoiceChannelId} channels={workspace.channels} connectedVoiceChannelId={voice.connectedChannelId} identity={localIdentity} mobileOpen={mobileNavigationOpen} onChannelChange={(channelId) => { workspace.setActiveChannelId(channelId); setActiveVoiceChannelId(null); setMobileNavigationOpen(false) }} onCloseMobile={() => setMobileNavigationOpen(false)} onCreateChannel={(kind) => { setChannelCreationKind(kind); setSettingsTab('channels'); setSettingsOpen(true) }} onOpenServerSettings={() => { setSettingsTab('server'); setSettingsOpen(true) }} onExit={onExit} onOpenPeople={() => setPeopleDialogOpen(true)} onVoiceChannelChange={(channelId) => { setActiveVoiceChannelId(channelId); setMobileNavigationOpen(false) }} server={workspace.activeServer} unreadByChannel={workspace.unreadByChannel} voiceParticipantsByChannel={voice.participantsByChannel} />
        {activeVoiceChannel ? <LivePanel channel={activeVoiceChannel} connected={voice.connectedChannelId === activeVoiceChannel.id} connecting={voice.connecting} demoMode={demoMode} onJoin={() => joinVoiceChannel(activeVoiceChannel)} participants={voice.participantsByChannel[activeVoiceChannel.id] ?? []} screenTrack={voice.screenTrack} stream={voice.stream} /> : <ChatPanel activeChannel={workspace.activeChannel} identity={localIdentity} loading={workspace.loading} messages={workspace.messages} onOpenMobileNavigation={() => setMobileNavigationOpen(true)} onSendMessage={workspace.sendMessage} server={workspace.activeServer} userId={userId} />}</> : directFriend ? <DirectMessagePanel demoMode={demoMode} friend={directFriend} identity={localIdentity} onBack={() => setDirectFriend(null)} userId={userId} /> : <FriendsHome friendRequests={workspace.friendRequests} friends={workspace.friends} onAddFriend={() => setPeopleDialogOpen(true)} onOpenFriend={setDirectFriend} />}
