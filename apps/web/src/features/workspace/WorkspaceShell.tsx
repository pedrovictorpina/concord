  ...
  return (
    <main className={workspace.activeServer ? 'app-shell' : 'app-shell home-shell'}>
      ...
      {workspace.activeServer ? <><ChannelPanel activeChannelId={workspace.activeChannelId} activeVoiceChannelId={activeVoiceChannelId} channels={workspace.channels} connectedVoiceChannelId={voice.connectedChannelId} identity={localIdentity} mobileOpen={mobileNavigationOpen} onChannelChange={(channelId) => { workspace.setActiveChannelId(channelId); setActiveVoiceChannelId(null); setMobileNavigationOpen(false) }} onCloseMobile={() => setMobileNavigationOpen(false)} onCreateChannel={(kind) => { setMobileNavigationOpen(false); setChannelCreationKind(kind); setSettingsTab('channels'); setSettingsOpen(true) }} onLeaveServer={() => { setMobileNavigationOpen(false); void workspace.leaveServer() }} onMarkServerRead={() => { setMobileNavigationOpen(false); void workspace.markServerRead() }} onModerateMember={workspace.moderateMember} onSetParticipantVolume={voice.setParticipantVolume} onOpenInvite={() => { setMobileNavigationOpen(false); setInviteFriendsOpen(true) }} onOpenPermissions={() => { setMobileNavigationOpen(false); setSettingsTab('permissions'); setSettingsOpen(true) }} onOpenServerSettings={() => { setMobileNavigationOpen(false); setSettingsTab('server'); setSettingsOpen(true) }} onToggleMuted={() => { void workspace.setMuted(!workspace.serverMuted) }} serverMuted={workspace.serverMuted} onOpenPeople={() => { setMobileNavigationOpen(false); setPeopleDialogOpen(true) }} onVoiceChannelChange={(channelId) => { setActiveVoiceChannelId(channelId); workspace.setActiveChannelId(channelId); setMobileNavigationOpen(false); const channel = workspace.channels.find((item) => item.id === channelId); if (channel && voice.connectedChannelId !== channelId) joinVoiceChannel(channel) }} server={workspace.activeServer} unreadByChannel={workspace.unreadByChannel} userId={userId} voiceParticipantsByChannel={voice.participantsByChannel} volumeByUser={voice.volumeByUser} />
        {activeVoiceChannel ? <LivePanel channel={activeVoiceChannel} connected={voice.connectedChannelId === activeVoiceChannel.id} connecting={voice.connecting} identity={localIdentity} messages={workspace.messages} onJoin={() => joinVoiceChannel(activeVoiceChannel)} onSendMessage={workspace.sendMessage} onSetParticipantVolume={voice.setParticipantVolume} onToggleShareSound={voice.setScreenAudioSilenced} onWatchShare={voice.setScreenShareWatched} screenAudioMuted={voice.screenAudioMuted} participants={voice.participantsByChannel[activeVoiceChannel.id] ?? []} screenShares={voice.screenShares} userId={userId} volumeByUser={voice.volumeByUser} /> : <ChatPanel activeChannel={workspace.activeChannel} identity={localIdentity} loading={workspace.loading} messages={workspace.messages} onOpenMobileNavigation={() => setMobileNavigationOpen(true)} onSendMessage={workspace.sendMessage} server={workspace.activeServer} userId={userId} />}
