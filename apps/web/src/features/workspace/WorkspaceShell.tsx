  ...
  return (
    <main className={workspace.activeServer ? 'app-shell' : 'app-shell home-shell'}>
      ...
      {workspace.activeServer ? <><ChannelPanel activeChannelId={workspace.activeChannelId} activeVoiceChannelId={activeVoiceChannelId} channels={workspace.channels} identity={localIdentity} mobileOpen={mobileNavigationOpen} onChannelChange={(channelId) => { workspace.setActiveChannelId(channelId); setMobileNavigationOpen(false) }} onCloseMobile={() => setMobileNavigationOpen(false)} onCreateChannel={(kind) => { setChannelCreationKind(kind); setSettingsTab('channels'); setSettingsOpen(true) }} onExit={onExit} onOpenPeople={() => setPeopleDialogOpen(true)} onVoiceChannelChange={(channelId) => { setActiveVoiceChannelId(channelId); setVoiceJoinRequest((current) => current + 1); setMobileNavigationOpen(false) }} server={workspace.activeServer} unreadByChannel={workspace.unreadByChannel} voiceParticipantChannelId={voiceParticipantChannelId} voicePanel={<LivePanel demoMode={demoMode} joinRequest={voiceJoinRequest} onConnectionChange={(channelId, connected) => setVoiceParticipantChannelId(connected ? channelId : null)} voiceChannel={activeVoiceChannel} />} />
        <ChatPanel activeChannel={workspace.activeChannel} identity={localIdentity} loading={workspace.loading} messages={workspace.messages} onOpenMobileNavigation={() => setMobileNavigationOpen(true)} onSendMessage={workspace.sendMessage} server={workspace.activeServer} userId={userId} /></> : <FriendsHome friendRequests={workspace.friendRequests} friends={workspace.friends} onAddFriend={() => setPeopleDialogOpen(true)} />}
