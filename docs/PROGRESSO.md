...
- sala de voz (`LivePanel`) ganhou cabecalho com contagem real de participantes (nunca uma metrica de qualidade inventada) e um botao para recolher o chat de voz, que agora libera de verdade a coluna reservada no grid (mesma tecnica de `members-collapsed`, reaproveitada tambem para esse caso).
- a sala de voz passou a ter sua propria barra de controles (mic/audio/tela/sair), replicando o que ja existia no `VoiceDock`; antes, a unica forma de mutar o microfone ou sair de dentro da propria tela do canal era o dock flutuante por cima do conteudo.
- para nao duplicar duas barras de controle na mesma tela, o `VoiceDock` passou a aceitar um modo compacto: quando o usuario ja esta vendo a sala do canal em que esta conectado, o dock se reduz a avisos reais (erro, aviso, audio bloqueado) e some por completo se nao houver nenhum; fora da sala (em outro canal, em outro servidor ou na Home) ele continua aparecendo por inteiro, do jeito que sempre funcionou.
...
- os tres design systems de `docs/design-system/` foram aplicados como familias visuais completas: `concord` (padrao) passou a ser o tema sobrio do documento 01 — Inter, canvas azulado `#0f172a`, verde `#22c55e`, bordas de 1px, radius 8/12 e sombras suaves, no lugar da versao neo-brutalista que ocupava esse id; `neo` (Neo Brutalism) recebeu a paleta oficial do documento 02 com verde neon `#00ff33`, roxo `#bb00ff`, bordas de 2–3px e sombras solidas sem blur; `glass` (Liquid Glass) recebeu a paleta do documento 03 com superficies translucidas, `backdrop-filter`, highlights internos e radius amplos.
- bug de tema corrigido no caminho: `--space-*`, `--font-size-*` e `--texture-opacity` eram declarados apenas sob `:root[data-style-theme='concord']`, entao qualquer outra familia deixava ~140 referencias `var(--space-*)`/`var(--font-size-*)` sem valor e colapsava paddings, gaps e tamanhos de fonte. Os tokens que nao mudam entre familias foram para `apps/web/src/theme/tokens/base.css` em `:root`, junto com escalas novas (z-index, avatar, altura de controle, tipografia) que o design system pede.
- os ids `ios` e `brutal` foram renomeados para `glass` e `neo`, como o proprio design system especifica. Como o id fica salvo em `localStorage`, a leitura migra os valores antigos em `theme-types.ts` e o bootstrap inline do `index.html` repete a migracao, para o tema de quem ja havia escolhido nao voltar ao padrao nem piscar errado no primeiro paint.
- efeitos que pertencem a uma familia ficaram no CSS dela, sem componente novo: o movimento fisico dos botoes do Neo Brutalism (deslocamento contra a propria sombra, com offset reduzido no mobile para nao gerar scroll horizontal) e o vidro do Liquid Glass, restrito a paineis, cabecalhos, composer, docks e menus — mensagens e listas longas continuam sobre superficie estavel, com `@supports` de fallback opaco e reducao de blur no mobile, por legibilidade e por nao competir com voz e compartilhamento de tela.
- o rodape da tela de entrada mostrava "Concord Neo" fixo em codigo; passou a ler o rotulo do tema ativo no registro. A unica cor fixa que ainda existia em CSS de componente (`rgb(0 0 0 / 20%)` na sombra do `.hint`) virou token, entao a camada de cor esta inteiramente tokenizada.
- validacoes: `pnpm check`, `pnpm lint` e `pnpm test:e2e` (26/26, com o teste de estilo atualizado de `iOS Glass`/`ios` para `Liquid Glass`/`glass`) aprovados; conferencia visual manual via Playwright nas seis combinacoes (`concord`/`neo`/`glass` x claro/escuro) em chat e configuracoes, checando ausencia de overflow horizontal, e teste temporario confirmando a migracao `ios` -> `glass` e `brutal` -> `neo`.
